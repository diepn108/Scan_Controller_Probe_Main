...
class VNA_Plugin(ProbePlugin):
    def __init__(self):
        ...
        self.vna_model = PluginSettingString("VNA Model", "37397C", select_options=["37397C", "37347C"], restrict_selections=True)
        ...
        self.freq_mode = PluginSettingString("Frequency Mode Query or Write", "Query", select_options=["Query", "Write"], restrict_selections=True)
        ...
        self.error_correction = PluginSettingString("Error Correction Mode", "Query", select_options=["Query", "On", "Off"], restrict_selections=True)
        self.data_mode = PluginSettingString("Data Mode (Raw or Corrected)", "Corrected", select_options=["Raw", "Corrected"], restrict_selections=True)
    ...
    def connect(self):
        try:
            ...
            raise RuntimeError(f"Failed to initialize VISA connection: {str(e)}")
        ...
        if self.freq_mode.value == "Query":
            ...
            if ec_status == "1" and (self.error_correction.value == "Query" or self.error_correction.value == "On"):
                cal_start = float(self.vna.query("CSF?"))
                cal_stop = float(self.vna.query("CTF?"))
                if self.freq_start.value < cal_start or self.freq_stop.value > cal_stop:
                    print(f"Warning: New frequency range ({self.freq_start.value} Hz to {self.freq_stop.value} Hz) "
                          f"exceeds calibration range ({cal_start} Hz to {cal_stop} Hz). Disabling error correction.")
                    self.vna.write("COF")  # Automatically disable error correction if range mismatch
            ...
            print(f"Setting start frequency to {self.freq_start.value} Hz")
            self.vna.write(f"SRT {self.freq_start.value} HZ")
            print(f"Setting stop frequency to {self.freq_stop.value} Hz")
            ...
            print(f"Setting IF bandwidth to {ifbw} Hz")
            if ifbw <= 10:
                self.vna.write("IF1")
            elif ifbw <= 100:
                self.vna.write("IF2")
            ...
            raw_start = self.vna.query("SRT?")
            raw_stop = self.vna.query("STP?")
            print(f"Verified start frequency: {float(raw_start.strip())} Hz")
            print(f"Verified stop frequency: {float(raw_stop.strip())} Hz")
            if float(raw_start.strip()) != self.freq_start.value or float(raw_stop.strip()) != self.freq_stop.value:
                print("Warning: VNA did not accept the new frequency settings. Check calibration or error correction.")

        else:
            raise ValueError("Incorrect Input on frequency mode setting")
        ...
        if not (opc_done == "1"):
            raise RuntimeError(f"Error, Opc returned unexpected value while waiting for a single sweep to finish (expected '1', received {opc_done})")
            self.vna.close()
    ...
    def disconnect(self):
        if self.vna:
            try:
                ...
                print(f"Error closing VNA connection: {str(e)}")
            self.vna = None
        ...
        for name in self.get_channel_names():
            ...
            if cmd:
                try:
                    ...
                    results[name] = np.array([complex(vals[i], vals[i+1]) for i in range(0, len(vals), 2)])
                except Exception as e:
                    print(f"Error reading {name} data: {str(e)}")
                    raise
        return results
